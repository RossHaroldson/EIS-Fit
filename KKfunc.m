...
% Boukamp's equivalent circuit (1995) plus an inductor in series
    ...
    M=N;
    Tauk = 1./logspace(log10(max(w)),log10(min(w)),M)';
    % weights = data.Zmod.^(-2);
    ...
    Zfunc = @(V,w) KKvec(V,Tauk,w)./sqrt((real(Zdata).^2+imag(Zdata).^2));
...
for i = 1:length(w)
    fitvec(i) =  V(1) + sum( V(3:end-1)./(1+(w(i).*Tauk(3:end-1)).^2)) + ...
        1i.*(-V(2)./w(i) - V(end).*w(i) - sum(V(3:end-1).*w(i).*Tauk(3:end-1)./(1+(w(i).*Tauk(3:end-1)).^2)));
end
end