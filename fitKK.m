function [] = fitKK(data) % freq, Zdata, Zcircuit, elementVals
...
Zreal = data.Zreal;
Zimag = data.Zimag;
Zdata = Zreal + 1i*Zimag;
weights = 1./(data.Zmod.^2);
...
% -> Zcircuit([elementVals ',' freq])
    ...
    Zfit = ZrealKK + 1i*ZimagKK;
    
    Zres = Zdata-Zfit;
    
    R2real = 1 - sum(real(Zres).^2)/sum(real(Zdata-mean(Zdata)).^2);
    R2imag = 1 - sum(imag(Zres).^2)/sum(imag(Zdata-mean(Zdata)).^2);
    R2mod = 1 - sum(abs(Zres).^2)/sum(abs(Zdata-mean(Zdata)).^2);
    R2phase = 1 - sum(phase(Zres).^2)/sum(phase(Zdata-mean(Zdata)).^2);
    ...
    clf,cla
    subplot(2,2,1)
    semilogx(freq,real(Zres./Zdata),'.');
    xlim([freq(end),freq(1)]);
    xlabel('Freq');
    ylabel('Normalized Re(Z) Residuals');
    title(['$R_{\mathrm{re}}^2$ = ',num2str(R2real)],'Interpreter','latex');
    
    subplot(2,2,2)
    semilogx(freq,imag(Zres./Zdata),'.');
    xlim([freq(end),freq(1)]);
    xlabel('Freq');
    ylabel('Normalized Im(Z) Residuals');
    title(['$R_{\mathrm{im}}^2$ = ',num2str(R2imag)],'Interpreter','latex');
    
    subplot(2,2,3)
    semilogx(freq,abs(Zres./Zdata),'.');
    xlim([freq(end),freq(1)]);
    xlabel('Freq');
    ylabel('Normalized Z Residuals');
    title(['$R_{\mathrm{mod}}^2$ = ',num2str(R2mod)],'Interpreter','latex');
    
    subplot(2,2,4)
    semilogx(freq,phase(Zres./Zdata),'.');
    xlim([freq(end),freq(1)]);
    xlabel('Freq');
    ylabel('Normalized Phase Residuals');
    title(['$R_{\mathrm{ph}}^2$ = ',num2str(R2phase)],'Interpreter','latex');
